...
export class SurgeUIService extends BaseService<SurgeUI> {
  ...
  public async getVault(props: GetVaultProps) {
    const { SurgeUI, oracleAddress, vaultAddress, userAddress } = props
    const SurgeUIContract = this.getContractInstance(SurgeUI)
    if (userAddress) {
      return await SurgeUIContract.getVaultWithUser(vaultAddress, oracleAddress, userAddress)
    } else {
      return await SurgeUIContract.getVault(vaultAddress, oracleAddress)
    }
  }
}
